...
import { Send, Sparkles, AlertCircle, Copy, Check, Mail, Phone, Download, Eye } from 'lucide-react';
...
const Contact: React.FC = () => {
  ...
  // PDF file path
  const RESUME_PDF_PATH = '/web/assets/Sean_Zeng_Resume.pdf';
  ...
  const handleDownloadResume = () => {
    console.log('Downloading resume from:', RESUME_PDF_PATH);
    const link = document.createElement('a');
    link.href = RESUME_PDF_PATH;
    link.download = 'Sean_Zeng_Resume.pdf';
  ...
  const handlePreviewResume = () => {
    console.log('Opening resume preview:', RESUME_PDF_PATH);
  ...
  const decorativeStats = [
    ...
    { label: t.contact.stats.location, value: "CN/SHENZHEN" },
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              ...
              >
                ...
                <div className="relative z-10 flex items-center justify-between p-6 gap-4 w-full">
                  <div className="flex items-center gap-6 overflow-hidden">
                    <div className="w-12 h-12 rounded-full bg-white/10 group-hover:bg-black flex items-center justify-center transition-colors duration-300 shrink-0">
                      <Download className="w-6 h-6 text-white group-hover:text-neon transition-colors" />
                    </div>
                    <div className="flex flex-col items-start overflow-hidden">
                      <span className="font-mono text-xs text-gray-500 group-hover:text-black/60 tracking-widest uppercase transition-colors mb-1">
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-6 font-mono scrollbar-hide" ref={scrollRef}>
            {messages.map((msg, idx) => (
              <div key={idx} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[90%] p-4 text-sm md:text-base font-bold rounded-[1.5rem] leading-relaxed ${msg.role === 'user'
